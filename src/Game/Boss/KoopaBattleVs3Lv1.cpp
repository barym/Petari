...
KoopaBattleVs3Lv1::KoopaBattleVs3Lv1(Koopa* pKoopa)
    : KoopaBattleBase("クッパ戦闘（Ｖｓ３Ｌｖ１）", pKoopa), mStateChaseRoll(nullptr), mStateAttackShockWave(nullptr) {
}
...
void KoopaBattleVs3Lv1::init() {
    ...
    mStateChaseRoll = new KoopaStateChaseRoll(getHost());
    mStateAttackShockWave = new KoopaStateAttackShockWave(getHost());

    MR::initActorState(this, mStateChaseRoll, &NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvChaseRoll::sInstance, "ChaseRoll");
    MR::initActorState(this, new KoopaStateAttackFireShort(getHost()), &NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvAttackFire::sInstance,
                       "AttackFire");
    ...
    MR::initActorState(this, new KoopaStateJumpAway(getHost()), &NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvJumpAway::sInstance, "JumpAway");
...
void KoopaBattleVs3Lv1::exeDamageReverse() {
    ...
    if (mStateDamageEscape->update()) {
        ...
        if (KoopaFunction::isKoopaAngry(getHost())) {
            KoopaFunction::changeBgmStateNormal(0x78);
            kill();
        } else {
...
bool KoopaBattleVs3Lv1::tryCalcAndSetBaseMtx() {
    if (isNerve(&NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvChaseRoll::sInstance)) {
        if (mStateChaseRoll->tryCalcAndSetBaseMtx()) {
            return true;
        }
    }
...
void KoopaBattleVs3Lv1::attackSensor(HitSensor* pSender, HitSensor* pReceiver) {
    if (isNerve(&NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvChaseRoll::sInstance)) {
        ...
        if (isNerve(&NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvJumpAway::sInstance)) {
...
bool KoopaBattleVs3Lv1::receiveMsgPlayerAttack(u32 msg, HitSensor* pSender, HitSensor* pReceiver) {
    if (isNerve(&NrvKoopaBattleVs3Lv1::KoopaBattleVs3Lv1NrvChaseRoll::sInstance)) {
        if (mStateChaseRoll->tryDamage(msg, pSender, pReceiver)) {
